...
class SourceEntryExtractorTest {
    ...
    void namedEntriesFollowTheChosenPresets() {
        ...
        docs.put(NexonEndpoint.SYMBOL_EQUIPMENT, json("""
                {"symbol":[{"symbol_name":"아케인심볼 : 소멸의 여로","symbol_level":20}]}"""));
        ...
        docs.put(NexonEndpoint.SKILL_0, json("""
                {"character_skill":[{"skill_name":"달팽이 세마리","skill_level":2,"skill_effect":"MP 5 소비하여 데미지 25"},
                                    {"skill_name":"Lv.2 궁디팡팡 멍뭉이","skill_level":1,"skill_effect":"공격력 20, 마력 20증가"}]}"""));

        Map<String, Map<String, String>> entries = extractor.extract(
                docs, MAPPER.createArrayNode(), new PresetSelection(1, 3, 2, 1), "나이트로드", "스카니아");

        assertThat(entries.get("symbol")).containsEntry("아케인심볼 : 소멸의 여로", "Lv.20");
        assertThat(entries.get("hyperStat")).containsOnly(Map.entry("보스 몬스터 공격 시 데미지 증가", "Lv.12"));
        assertThat(entries.get("ability")).containsEntry("어빌리티 1", "보스 몬스터 공격 시 데미지 20% 증가");
        assertThat(entries.get("unionRaider")).containsEntry("LUK 증가", "100");
        assertThat(entries.get("unionOccupied")).containsEntry("보스 몬스터 공격 시 데미지 증가", "5.00%");
        assertThat(entries.get("unionArtifact")).containsEntry("올스탯 증가", "150 (Lv.10)");
        assertThat(entries.get("hexaStat")).containsEntry("코어1 주 주력 스탯 증가", "Lv.5");
        // 전투력에 안 들어가는 달팽이 세마리는 빠지고, 펫 버프 모양의 스킬만 남는다
        assertThat(entries.get("skill")).containsOnlyKeys("Lv.2 궁디팡팡 멍뭉이");
    }
