...
//    );
//
//    private void applyMainStat(String name, Integer level, StatSheet statSheet) {
//        switch(name) {
//            case "보스 데미지 증가" -> statSheet.setBOSS_DAMAGE((int) (statSheet.getBOSS_DAMAGE() + MAIN_STAT.get(name)[level-1]));
//            case "크리티컬 데미지 증가" -> statSheet.setCRITICAL_DAMAGE((statSheet.getCRITICAL_DAMAGE() + MAIN_STAT.get(name)[level-1]));
//            case "데미지 증가" -> statSheet.setDAMAGE((int) (statSheet.getDAMAGE() + MAIN_STAT.get(name)[level-1]));
//            case "공격력 증가" -> statSheet.setATTACK_POWER((int) (statSheet.getBOSS_DAMAGE() + MAIN_STAT.get(name)[level-1]));
//            case "주력 스탯 증가" -> statSheet.setSTR((int) (statSheet.getBOSS_DAMAGE() + MAIN_STAT.get(name)[level-1]));
//
//        }
//    }
//
//    public StatSheet getCurrentPresetAbility(JsonNode hexa) {
