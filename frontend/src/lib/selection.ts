...
/**
 * 기본 구간: 값이 있는 최신 두 지점 — 끝은 항상 오늘이다.
 *
 * <p>전에는 "전투력이 마지막으로 변한 인접 두 지점"을 골랐다. 며칠째 그대로인 캐릭터를
 * 열면 처음 보이는 구간이 며칠 전으로 밀려 있어서, 지금 상태를 보러 왔는데 과거를 보고
 * 있는 셈이 됐다. 변한 곳은 차트의 핀이 이미 가리킨다.
 * 값이 있는 지점이 둘 미만이면 null.
 */
